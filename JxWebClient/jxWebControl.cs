...
{
    public class logonDetectionData
    {
    ...
    public class logonAppDetectionData
    {
    ...
    public class jxWebControl
    {
        ...
        public bool sendAuthorizationDataWithVersion()
        {
            ...
            return ex.sendAuthorizationData(user, password);
        }
        ...
        public bool logon(logonDetectionData logondata, logonAppDetectionData logonappdata, out string code, out string message)
        {
            message = "";
            try
            {
                string jsonstring = new JObject(
                    new JProperty("detectionData", new JObject((JObject)JsonConvert.DeserializeObject(ObjectToJson(logondata)))),
                    new JProperty("appDetectionData", new JObject((JObject)JsonConvert.DeserializeObject(ObjectToJson(logonappdata))))
                    ).ToString();
                ini.INIIO.saveSocketLogInf("SEND:\r\nlogon(" + jsonstring + ")");
                string ack = ex.logon( jsonstring);
                if (ack == null)
                {
                    sendAuthorizationDataWithVersion();
                    ack = ex.logon(jsonstring);
                }
                ini.INIIO.saveSocketLogInf("RECEIVE:\r\n" + ack);
                JObject ja = (JObject)JsonConvert.DeserializeObject(ack);
                code = ja["resultCode"].ToString();
        ...
        public void takePhotoJx(string ip,string port,string detectionID,int lx)
        {
            try
            {
                ...
                socket.Send(Encoding.ASCII.GetBytes(detectionID + "," + lx.ToString()));
                socket.Close();
