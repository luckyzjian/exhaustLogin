...
{
    public partial class carLoginHN : Form
    {
        ...
        private void init_loginDefault()
        {
            ...
            else if (mainPanel.isNetUsed && mainPanel.netMode == mainPanel.HNNETMODE)
            {
                ...
                IDictionaryEnumerator myEnumerator = mainPanel.hninterface.HN_HPZL.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                ...
                myEnumerator = mainPanel.hninterface.HN_SYXZ.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxSYXZ.Items.Add((string)(myEnumerator.Value));
                }
                myEnumerator = mainPanel.hninterface.HN_CSYS.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxCSYS.Items.Add((string)(myEnumerator.Value));
                }
                myEnumerator = mainPanel.hninterface.HN_RLZL.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxRLZL.Items.Add((string)(myEnumerator.Value));
                }
                myEnumerator = mainPanel.hninterface.HN_BSQXS.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxBSQXS.Items.Add((string)(myEnumerator.Value));
                }
                myEnumerator = mainPanel.hninterface.HN_GYXS.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxGYFS.Items.Add((string)(myEnumerator.Value));
                }
                ...
                myEnumerator = mainPanel.hninterface.HN_JQFS.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxJQFS.Items.Add((string)(myEnumerator.Value));
                }
                ...
                myEnumerator = mainPanel.hninterface.HN_JYZL.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxJCLB.Items.Add((string)(myEnumerator.Value));
                }
                ...
                comboBoxJHZZ.Items.Add("无");
                comboBoxJHZZ.Items.Add("有");
                dt = logininfcontrol.getComBoBoxItemsInf("是否选择");
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    comboBoxDLSP.Items.Add(dt.Rows[i]["MC"].ToString());
            ...
            if (!isnhcarinfexist)
            {
                ...
                else
                {
                    ...
                    if (model.CLHP != "-2")
                    {
                        ...
                        if (mainPanel.isNetUsed && (mainPanel.netMode == mainPanel.NHNETMODE||mainPanel.netMode==mainPanel.JXNETMODE||mainPanel.netMode==mainPanel.HNNETMODE))
                        {
                            //comboBoxCPYS.Text = model.CPYS;
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            if (true)//如果参检环保
            {
                ...
                else if (mainPanel.isNetUsed && mainPanel.netMode == mainPanel.HNNETMODE)
                {
                    ...
                    model.HPZL = comboBoxHPZL.Text;
                    model.ZXBZ = comboBoxHPZL.Text;
                    logininfcontrol.saveCarInfbyPlateHN(model);
                    waitmodel.CLHP = comboBoxCLPH.Text;
                    
                    waitmodel.CLID = textBoxCLID.Text;
                    waitmodel.DLSJ = loginTime;
                    waitmodel.JCCS = textBoxJCCS.Text.Trim();
                    waitmodel.CPYS = comboBoxCPYS.Text;
                    waitmodel.DLY = mainPanel.nowUser.userName;
        ...
        private void buttonPlate_Click(object sender, EventArgs e)
        {
            ...
            if (mainPanel.isNetUsed && mainPanel.netMode == mainPanel.HNNETMODE)
            {
                ...
                ht.Add("hphm", comboBoxCLPH.Text);
                ht.Add("hpzl", mainPanel.hninterface.RHN_HPZL.GetValue(comboBoxHPZL.Text, ""));
                ht.Add("clsbdh", textBoxDPHM.Text);
                ht.Add("jczl", mainPanel.hninterface.RHN_JYZL.GetValue(comboBoxJCLB.Text, ""));
                ...
                if (mainPanel.hninterface.queryVehicleInf(ht, out dtinf, out code, out msg))
                {
                    if (dtinf != null)
                    {
                        ...
                        {
                            DateTime a, b;
                            textBoxCLID.Text = dtinf.Rows[0]["JCLSH"].ToString();
                            if (dtinf.Columns.Contains("CLXH"))
                            {
                                textBoxDPHM.Text= dtinf.Rows[0]["CLSBDH"].ToString();
                                comboBoxXH.Text = dtinf.Rows[0]["XH"].ToString();
                                textBoxPP.Text = dtinf.Rows[0]["CLXH"].ToString();                               
                                textBoxFDJXH.Text = dtinf.Rows[0]["FDJXH"].ToString();
                                textBoxFDJH.Text = dtinf.Rows[0]["FDJH"].ToString();
                                comboBoxCLLX.Text = dtinf.Rows[0]["CLLX"].ToString() + "_" + logininfcontrol.getComBoBoxItemsNAME("车辆类型", dtinf.Rows[0]["CLLX"].ToString());
                                comboBoxCSYS.Text = mainPanel.hninterface.HN_CSYS.GetValue(dtinf.Rows[0]["CSYS"].ToString(), dtinf.Rows[0]["CSYS"].ToString());
                                comboBoxSYXZ.Text = mainPanel.hninterface.HN_SYXZ.GetValue(dtinf.Rows[0]["SYXZ"].ToString(), "");
                                textBoxCZ.Text = dtinf.Rows[0]["SYR"].ToString();
                                dateZCRQ.Value = DateTime.Parse(dtinf.Rows[0]["CCDJRQ"].ToString());
                                comboBoxRLZL.Text = mainPanel.hninterface.HN_RLZL.GetValue(dtinf.Rows[0]["RLZL"].ToString(), "");
                                textBoxFDJPL.Text = dtinf.Rows[0]["PL"].ToString();
                                textBoxEDGL.Text = dtinf.Rows[0]["GL"].ToString();
                                textBoxZZL.Text = dtinf.Rows[0]["ZZL"].ToString();
                                textBoxZBZL.Text = dtinf.Rows[0]["ZBZL"].ToString();
                                dateSCRQ.Value = DateTime.Parse(dtinf.Rows[0]["CCRQ"].ToString());
                            }
                            else
                            {
                                if (MessageBox.Show("在平台未获取到该车相关信息，是否继续登录？", "系统提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.Cancel)
                                {
                                    return;
                                }
                            }                      
                ...
                {
                    if (MessageBox.Show("获取车辆注册信息失败\r\ncode:" + code + "\r\nmsg:" + msg+"\r\n是否继续登录？", "系统提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.Cancel)
                    {
        ...
        {
            if (!mainPanel.isNetUsed || (mainPanel.netMode != mainPanel.NHNETMODE&& mainPanel.netMode != mainPanel.JXNETMODE))
            {
                DataTable dt = null;
