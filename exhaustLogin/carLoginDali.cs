...
{
    public partial class carLoginDali : Form
    {
        ...
        private void init_loginDefault()
        {
            ...
            else if(mainPanel.isNetUsed&&mainPanel.netMode==mainPanel.DALINETMODE)
            {
                ...
                myEnumerator = mainPanel.daliinterface.DL_SCD.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                ...
                myEnumerator = mainPanel.daliinterface.DL_CZLB.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxOBD.Items.Add((string)(myEnumerator.Value));
                }
                myEnumerator = mainPanel.daliinterface.DL_YYXZ.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
                    comboBoxSRL.Items.Add((string)(myEnumerator.Value));
                }
                myEnumerator = mainPanel.daliinterface.DL_YZZL.GetEnumerator();
                while (myEnumerator.MoveNext())//遍历哈希表的值,生成xml节点
                {
            ...
            if (!isnhcarinfexist)
            {
                ...
                else
                {
                    ...
                    if (model.CLHP != "-2")
                    {
                        ...
                        if (mainPanel.isNetUsed && (mainPanel.netMode == mainPanel.NHNETMODE||mainPanel.netMode==mainPanel.JXNETMODE||mainPanel.netMode==mainPanel.DALINETMODE))
                        {
            ...
            if (true)//如果参检环保
            {
                ...
                foreach (Control control in groupBox1.Controls)
                {
                    if (control is TextBox || control is ComboBox)
                    {
                        ...
                        if (control.Text == ""&&control.Name!= "textBoxJSSZK")
                        {
                            MessageBox.Show("请填写完所有车辆信息后再保存", "系统提示");
                            return;
                        }
                ...
                if (textBoxJSSZK.Text == "" && comboBoxOBD.Text == "个人")
                {
                    MessageBox.Show("车主类别为个人时，必需填写身份证号", "系统提示");
                    return;
                }
                ...
                if (mainPanel.isNetUsed && mainPanel.netMode == mainPanel.NHNETMODE)
                {
                    ...
                    if (comboBoxRLZL.Text.Contains("柴油"))
                    {
                        ...
                        else if (!comboBoxQDXS.Text.Contains("全驱") && comboBoxCHZZ.Text != "不可摘除" && DateTime.Compare(model.SCRQ, Convert.ToDateTime("2001-10-01")) >= 0)//2001年10月1日 起生产的汽车，轻型车额定功率不超过150KW，重型车额定功率不超过350KW
                        {
                    ...
                    {
                        if (!comboBoxQDXS.Text.Contains("全驱") && comboBoxCHZZ.Text != "不可摘除")
                        {
                            if (int.Parse(model.ZZL) > 3500)
                            {
